...
#define SPIN_LOCK_LOCK(x)                                              \
    do {                                                               \
        ...
        while (atomic_flag_test_and_set_explicit(&x, memory_order_acquire)) { \
            for (int __spin_i = 0; __spin_i < __spin_backoff; ++__spin_i) \
                intr_pause();                                          \
...
#define DELAY(x) do { for (size_t i = 0; i < x; ++i) { intr_pause(); } }while(0) 

#define PAGE_SIZE 4096

#define QUICKPOOL_DIVISIONS 8
#define QUICKPOOL_POOLS 4
#define QUICKPOOL_TOTAL 10
