...
static void insert_slab_sorted(skiplist_t *list, slab_header_t *header, bool nonblock) {
    ...
    printf("insert_slab_sorted ptr: %p, nonblock: %i\n", header, nonblock);
    ...
    for (int i = SKIP_LIST_MAX_LEVEL - 1; i >= 0; i--) {
        while ((slab_header_t *)current->next[i] != NULL && (slab_header_t *)current->next[i] < header) {
            if (current == (void*)current->next[i]) {
                printf("slab_skip_list: duplicate node %p\n", current);
                exit(1);
            }
    ...
    for (uint32_t i = 0; i < level; i++) {
        //printf("Updating: %p\n", update[i]);
        slab_header_t* next = (slab_header_t*)update[i]->next[i];
        if (next == header) {
            printf("slab_skip_list: trying to create a loop on level %i %p\n", i, header);
            exit(1);
        }
...
static bool remove_slab(skiplist_t *list, slab_header_t *header, bool deleting, bool nonblock) {
    ...
    printf("remove_slab ptr: %p, nonblock: %i\n", header, nonblock);
...
void slab_free(void *ptr) {
    ...
    // We trust the page allocator to keep low pages for us. We won't
    // try to reuse slabs that run empty again
}
